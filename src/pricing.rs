...
fn cache_age(cache: &PricingCache, now: SystemTime) -> Option<Duration> {
...
fn read_cache(cache_path: &Path) -> Result<PricingCache> {
...
fn write_cache(cache_path: &Path, cache: &PricingCache) -> Result<()> {
...
fn fetch_pricing_cache() -> Result<PricingCache> {
...
fn fetch_pricing_cache_from(
    ...
    Ok(pricing_cache_from_dataset(dataset))
}
